...
function CustomCalendar() {
  ...
  // const [gapiLoadAttempts, setGapiLoadAttempts] = useState(0);
...
// Added 20th July 16:26
const [gapiLoadAttempts, setGapiLoadAttempts] = useState(0);

useEffect(() => {
  const MAX_RETRIES = 3;

  const loadGAPI = async (attempt = 1) => {
    try {
      ...
      if (attempt < MAX_RETRIES) {
        ...
        setGapiLoadAttempts(attempt + 1);
        return loadGAPI(attempt + 1);
      }
      console.error("Final initialization failed:", error);
      setAuthError("Failed to load Google services. Please refresh.");
    }
  };

  loadGAPI();
}, [gapiLoadAttempts]); // Retry when attempts change

  // removed 20th July 16:25
...
const initGoogleClient = async () => {
  try {
    ...
    // 1. Verify gapi is loaded
    if (!window.gapi) {
      throw new Error("Google API script not loaded");
    }

    // 2. Load client and auth2 libraries
    await new Promise((resolve, reject) => {
      window.gapi.load('client:auth2', {
        callback: resolve,
        onerror: reject,
    ...
    console.log("[2] gapi.client initialized successfully");
    ...
    console.log("[3] Auth instance obtained:", auth);
    console.log("[4] Current user:", auth.currentUser.get());
    console.log("[5] Is signed in:", auth.isSignedIn.get());
  ...
  const fetchGoogleEvents = async () => {
    if (!authInstance) 
      console.error("[FETCH ERROR] No auth instance");
      return;

    try {
      console.log("[FETCH] Requesting events...");
    ...
    // }
      ...
      console.error("[FETCH ERROR DETAILS]", {
        ...
        details: error.result ? error.result.error : null,
        stack: error.stack
      });
      setAuthError("Failed to fetch calendar events");
    }
...
function GoogleApiLoader() {
  ...
  return (
    <div className="api-status">
      {status === "loading" && <span>Loading Google API...</span>}
      {status === "failed" && (
        <button 
    ...
    <div className="calendar-wrapper">
      ...
      >
        ...
        <button 
            ...
            disabled={!isGoogleApiLoaded || isLoading}
            style={{
              background: isGoogleConnected ? '#4285F4' : '#34A853',
              color: 'white',
              padding: '10px 15px',
              border: 'none',
              borderRadius: '4px',
              cursor: (!isGoogleApiLoaded || isLoading) ? 'not-allowed' : 'pointer'
            }}
          >
            {isLoading ? (
