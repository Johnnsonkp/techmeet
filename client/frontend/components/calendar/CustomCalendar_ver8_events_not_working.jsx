...
useEffect(() => {
  ...
  const initClient = async () => {
    ...
    setAuthError(`Google Calendar initialization failed. Please refresh the page.`);
  }
};
  //     setAuthInstance(auth);
  //     auth.isSignedIn.listen(updateSigninStatus);
  ...
  //     setAuthError(`Failed to initialize Google Calendar: ${error.error || error.message}`);
  //   }
  // };

  window.gapi.client.load('calendar', 'v3', initClient);
}, [gapiLoaded]);
...
//     } catch (error) {
//       console.error('Error initializing Google client:', error);
//       setAuthError('Failed to initialize Google Calendar');
//     }
//   };
